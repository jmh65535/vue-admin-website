...
const responseHandler = ctx => {
    ...
    if (body !== undefined) {
        ctx.type = 'json';
        ctx.body = {
            code: !body.code ? 20000 : body.code,
            message: !body.msg ? 'success' : msgConfig[body.msg]
        }
        body.data && (ctx.body.data = body.data)
        body.token && (ctx.body.token = body.token)
    }
}
...
const errorHandler = (ctx, next) => {
    return next().catch(err => {
        ...
        console.log('==========ERRRRRRROR.end==========');
        if (err.code == null) {
            logger.error(err.stack);
        }
        ctx.body = {
            code: err.code || -1,
            message: err.message.trim() + ' from errorHandler',
            data: null,
        };
        // 保证返回状态是 200
        ctx.status = 200;
